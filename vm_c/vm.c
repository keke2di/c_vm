...
static int function_index_by_name(const VM *vm, uint32_t name_index) {
    for (uint32_t i = 0; i < vm->num_functions; ++i) {
        if (vm->functions[i].name_index == name_index) return (int)i;
    }
    return -1;
}
    ...
    while (1) {
        ...
        switch (op) {
            ...
            case OP_BINARY_MOD: {
                ...
                if (a->tag == TAG_INT && b->tag == TAG_INT) {
                    ...
                    if (bf == 0.0) {
                        ...
                        vm_push_owned(vm, value_new_float(fmod(af, bf)));
                    }
                } else {
                    vm->last_error = VM_ERR_TYPE;
                }
            ...
            case OP_CALL: {
                ...
                VM_DEBUG(
                    "[CALL] nargs=%u stack_before_name_pop=%u frame=%p base=%u\n",
                    nargs,
                    vm->stack_top,
                    (void *)vm->current_frame,
                    vm->current_frame ? vm->current_frame->stack_base : 0);
                ...
                if (name_val->tag != TAG_INT) {
                    value_release(name_val);
                    vm->last_error = VM_ERR_TYPE;
                    break;
                }
                uint32_t func_idx = (uint32_t)name_val->data.int_val;
                value_release(name_val);
                ...
                VM_DEBUG(
                    "[CALL] after name pop stack=%u nargs=%u\n",
                    vm->stack_top,
                    nargs);

                // Built-in: print
                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "print") == 0) {
                    builtin_print(vm, nargs);
                    vm_push_owned(vm, value_new_int(0));
                    break;
                }
                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "list") == 0) {
                    builtin_list(vm, nargs);
                    break;
                }

                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "append") == 0) {
                    builtin_append(vm, nargs);
                    break;
                }

                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "enumerate") == 0) {
                    builtin_enumerate(vm, nargs);
                    break;
                }

                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "int") == 0) {
                    builtin_int(vm, nargs);
                    break;
                }
                // Built-in: str
                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "str") == 0) {
                    builtin_str(vm, nargs);
                    break;
                }
                // Built-in: float
                if (func_idx < vm->num_names && strcmp(vm->names[func_idx], "float") == 0) {
                    builtin_float(vm, nargs);
                    break;
                }



                int resolved_idx = function_index_by_name(vm, func_idx);
                if (resolved_idx < 0) {
                    vm->last_error = VM_ERR_FUNC_NOT_FOUND;
                    break;
                }
                ...
                FuncEntry *func = &vm->functions[(uint32_t)resolved_idx];
                if (nargs != func->params_count) {
                    vm->last_error = VM_ERR_TYPE;
                    break;
                }
                ...
                for (uint32_t i = nargs; i > 0; i--) {
                    ...
                    uint32_t param_idx = i - 1;
                    if (param_idx < locals_cap) {
                        locals[param_idx] = arg;
                    } else {
                        value_release(arg);
                    }
                }
                ...
                VM_DEBUG(
                    "[CALL] args consumed stack=%u creating frame for %s\n",
                    vm->stack_top,
                    func->name_index < vm->num_names ? vm->names[func->name_index] : "?");

                uint32_t stack_base = vm->stack_top;
                ...
                new_frame->stack_base = stack_base;
